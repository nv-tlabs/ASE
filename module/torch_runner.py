...
class Runner:
    def __init__(self, algo_observer=None):
        ...
        #! from run.build_alg_runner
        #! register_builder in each factory
    ...
    def load_config(self, params):
        ...
        #! take algo sort (ase, amp, etc)
        self.algo_params = params['algo']               
        self.algo_name = self.algo_params['name']
        self.load_check_point = params['load_checkpoint']
        self.exp_config = None

        if self.seed:                                   #! set seed
            torch.manual_seed(self.seed)
            torch.cuda.manual_seed_all(self.seed)
            np.random.seed(self.seed)

        if self.load_check_point:                       #! load checkpoint if exists
            print('Found checkpoint')
            print(params['load_path'])
            self.load_path = params['load_path']

        self.model = self.model_builder.load(params)    #! load model builer and change class variable with params
        self.config = copy.deepcopy(params['config'])   #! self.config = params:config in cfg_train (train related parameters)
        
        self.config['reward_shaper'] = tr_helpers.DefaultRewardsShaper(**self.config['reward_shaper'])  #! select reward sort(linear, decreasing, etc) 
        self.config['network'] = self.model
        
        has_rnd_net = self.config.get('rnd_config', None) != None   #! special case for using rnd net, but not use 
        if has_rnd_net:
        ...
        if has_central_value_net:                                   #! special case for using central_value_net, but not use
    ...
    def load(self, yaml_conf):
        ...
        #! from run.runner.run(cfg_train)
        self.default_config = yaml_conf['params']               #! self.default_config = cfg_train in run.py
        self.load_config(copy.deepcopy(self.default_config))

        if 'experiment_config' in yaml_conf:                    #! for experiment setting, not required
    ...
    def run_train(self):
        ...
        #! from self.run() if training 
        print('Started to train')
        if self.algo_observer is None:                      #! algo_observer = RLGPUObserver
        ...
        if self.exp_config:
            ...
            while exp is not None:
                ...
                exp = self.experiment.get_next_config()
        else:                                                           #! default setting(no experiment arguments)
            self.reset()                                                #! reset, yet nothing happen
            self.load_config(self.default_config)                       #! load cfg_train 
            if 'features' not in self.config:
                self.config['features'] = {}
            self.config['features']['observer'] = self.algo_observer    #! resister self.observer
            #if 'soft_augmentation' in self.config['features']:
            #    self.config['features']['soft_augmentation'] = SoftAugmentation(**self.config['features']['soft_augmentation'])
            agent = self.algo_factory.create(self.algo_name, base_name='run', config=self.config)   #! algorithm_builder created
            if self.load_check_point and (self.load_path is not None):                              #! manipulate the case checkpoint existed 
                agent.restore(self.load_path)
            agent.train()                                                                           #!! Main start point to make model, agent, env(vec)
    ...
    def run(self, args):
        ...
        #! from run.runner.run(vargs)
        if 'checkpoint' in args and args['checkpoint'] is not None:
            if len(args['checkpoint']) > 0:
                self.load_path = args['checkpoint']

        if args['train']:
            self.run_train()                    #! if training, run this fn
        elif args['play']:
            print('Started to play')            #! if testing, run this fn
            player = self.create_player()
            player.restore(self.load_path)
            player.run()
        else:
            self.run_train()                    #! default setting for exceptional case